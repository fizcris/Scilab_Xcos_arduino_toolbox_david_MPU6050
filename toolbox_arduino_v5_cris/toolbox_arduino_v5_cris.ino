/* This file is meant to be used with the SCILAB arduino  
  ...
  setPwmFrequencyUNO(pin,divisor);
  
    DO NOT MODIFY pin 6, is the one on which rely all time functions in Arduino: i.e., if you change the frequency of this pins, function like delay() 
    or millis() will continue to work but at a different timescale (quicker or slower!!!)
 
 For pins 6 and 5:   (5 and 6 are together, can not modify individually)
     Divisor 	                Frequency
        1	        	62500 Hz
    	2 	 		7812.5 Hz
 ...
 For pins 9, 10, 11 and 3: (9 & 10 and 11 & 3 are together, can not modify individually)
      Divisor 	                Frequency
	1 	 		31372.55 Hz
	2	 		3921.16	Hz
	3 	 		979.527
 ...
 ****** Change PWM frecuency ARDUINO MEGA 2560***** 
 ...
 setPwmFrequencyMEGA2560(pin,divisior); 
...
DO NOT MODIFY pins, 13 & 4 is the one on which rely all time functions in Arduino: i.e., if you change the frequency of this pins, function like delay() or 
millis() will continue to work but at a different timescale quicker or slower!))
    Divisor 	                Frequency
        1 	 		62500 Hz
	2 	 		7812.5 Hz
	3 	 		976.5625 Hz  <--DEFAULT Diecimila bootloader
...
//  setPwmFrequencyUNO(11,7)         //  sets frequency PWM pin 11 to 30.610 Hz
//  setPwmFrequencyMEGA2560(8,4);    //  sets frequency PWM pin 8 to 122.55 Hz 
   ...
   //Note that with the driver Pololu VNH5019 the timmer1 is set to 20kHz, the changes in PWM frequency won't be
   //applicate.
...
void loop() {
  ...
  static int  pin =  3;    /* generic pin number             */
  ...
  pinMode(A0,INPUT);  //NECESSARY  to read current Motor 1
  pinMode(A1,INPUT);  //NECESSARY to read current Motor 2
  pinMode(A2,INPUT);  //NECESSARY  
  pinMode(A3,INPUT);  //NECESSARY 
      ...
      if (val>48 && val<53) {
        ...
        if (val==48 || val ==49){
            ...
            if (dcm==1){
             if(dcm1_mode==0){//L293 
              if(direction==1){
                 val=val*(1.56863);  //(80/51)    //Modification driver Pololu VNH 
                md.setM1Speed(val);              //Motor 1 Resol:-400-->400 but XCOS -255-->255
                //analogWrite(dcm1_pin1,val);
                //analogWrite(dcm1_pin2,0);
              } else {
                val=val*(1.56863);//(80/51)      //Modification driver Pololu VNH (reverse)
                md.setM1Speed(-val);            //Motor 1 Resol:-400-->400 but XCOS -255-->255
            ...
            if (dcm==2){
             if(dcm2_mode==0){//L293 
              if(direction==1){
                val=val*(1.56863);//(80/51)      //Modification driver Pololu VNH (reverse)
                md.setM2Speed(val);            //Motor 1 Resol:-400-->400 but XCOS -255-->255
    ...
    if ((mpuIntStatus & 0x10) || fifoCount == 1024 || fifoCount > packetSize ) {
        ...
        fifoCount -= packetSize;
            ...
            if (acq_launched == true){
              ypr[0]=ypr[0]; //  ypr[0]=ypr[0]* 180/M_PI;
              ypr[1]=ypr[1];
              ypr[2]=ypr[2];
              //data[0]= atan2(aa.y,aa.z) * 180 / M_PI; // angel x calculed by acelaration // Modification to read angle x by acelerations
